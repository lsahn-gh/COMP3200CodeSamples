...
namespace ls
{
    ...
    lstring lstring::operator+(const char* rhs) const
    {
        size_t rhsSize = strlen(rhs);
        lstring newString(mLength + rhsSize + 1);
        memcpy(newString.mChar, mChar, mLength);
        memcpy(newString.mChar + mLength, rhs, rhsSize);
        newString.mLength = mLength + rhsSize;
        newString.mChar[newString.mLength] = '\0';
